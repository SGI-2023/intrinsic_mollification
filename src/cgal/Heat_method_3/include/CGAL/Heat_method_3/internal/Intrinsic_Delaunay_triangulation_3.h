...
namespace CGAL
{
  namespace Heat_method_3
  {
    ...
    class Intrinsic_Delaunay_triangulation_3
    {
      ...
      typedef typename boost::graph_traits<TriangleMesh>::halfedges_size_type halfedges_size_type;
      ...
      typedef CGAL::dynamic_halfedge_property_t<Index> Halfedge_property_tag;
      typedef typename boost::property_map<TriangleMesh, Halfedge_property_tag>::type Halfedge_id_map;

    private:
      friend struct IDT_vertex_point_property_map<Self>;
      template <class IDT, class VDM>
      friend struct IDT_vertex_distance_property_map;
    ...
    public:
      ...
      Intrinsic_Delaunay_triangulation_3(const TriangleMesh &input_tm, enum MOLLIFICATION_TYPE m_mollification_type, double delta = 1e-4)
          : m_intrinsic_tm(), m_input_tm(input_tm), m_vpm(*this), hcm(get(Halfedge_coordinate_tag(), m_intrinsic_tm)), m_mollification_type(m_mollification_type), delta(delta)
      {
        broken_edges =
            m_mollification_type == LOCAL_CONSTANT_EPSILON ||
            m_mollification_type == LOCAL_ONE_BY_ONE_STEP ||
            m_mollification_type == LOCAL_ONE_BY_ONE_INTERPOLATION;
      ...
      Intrinsic_Delaunay_triangulation_3(const TriangleMesh &input_tm, VertexPointMap vpm, enum MOLLIFICATION_TYPE m_mollification_type, double delta = 1e-4)
          : m_intrinsic_tm(), m_input_tm(input_tm), m_vpm(*this), hcm(get(Halfedge_coordinate_tag(), m_intrinsic_tm)), m_mollification_type(m_mollification_type), delta(delta)
      {
        broken_edges =
            m_mollification_type == LOCAL_CONSTANT_EPSILON ||
    ...
    private:
      ...
      void
      mollify_constant(const double delta, bool scale_length = false)
      {
        ...
        double extra_length = epsilon * num_edges(m_intrinsic_tm);
        // update edge lengths
        for (edge_descriptor ed : edges(m_intrinsic_tm))
        {
          ...
          if (scale_length)
          {
            edge_lengths[i] *= total_length / (total_length + extra_length);
          }
        }
      }

      void
      mollify_local_constant(const double delta, bool scale_length = false)
      {
        // compute smallest length epsilon we can add to
        // all edges to ensure that the strict triangle
        // inequality holds with a tolerance of delta
        // we need to change the following code to loop over faces instead of edges and the 3 edges of each face
        ...
        for (face_descriptor f : faces(m_intrinsic_tm))
        {
          ...
          halfedge_descriptor hd2 = next(hd, m_intrinsic_tm);
          halfedge_descriptor hd3 = next(hd2, m_intrinsic_tm);
          Index i = get(halfedge_id_map, hd);
          Index j = get(halfedge_id_map, hd2);
          Index k = get(halfedge_id_map, hd3);
          double ineq1 = edge_lengths[j] + edge_lengths[k] - edge_lengths[i];
          double ineq2 = edge_lengths[i] + edge_lengths[k] - edge_lengths[j];
          double ineq3 = edge_lengths[i] + edge_lengths[j] - edge_lengths[k];
          double epsilon = (std::max)(0., delta - (std::min)((std::min)(ineq1, ineq2), ineq3));
          edge_lengths[i] += epsilon;
          edge_lengths[j] += epsilon;
          edge_lengths[k] += epsilon;
          extra_length += epsilon * 3;
        }
        if (scale_length)
        {
          for (halfedge_descriptor hd : halfedges(m_intrinsic_tm))
      ...
      void
      mollify_local_one_by_one_step(const double delta, bool scale_length = false)
      {
        // compute smallest length epsilon we can add to
        // all edges to ensure that the strict triangle
        // inequality holds with a tolerance of delta
        ...
        for (face_descriptor f : faces(m_intrinsic_tm))
        {
          ...
          // reorder i, j, k so that edge_lengths[k] <= edge_lengths[j] <= edge_lengths[i]
          Index L_index[3] = {i, j, k};
          if (edge_lengths[L_index[0]] > edge_lengths[L_index[1]]) // now L_index[0] <= L_index[1]
          {
            std::swap(L_index[0], L_index[1]);
          }
          if (edge_lengths[L_index[1]] > edge_lengths[L_index[2]]) // now L_index[0] might be > L_index[1]
          {
            std::swap(L_index[1], L_index[2]);
          }
          if (edge_lengths[L_index[0]] > edge_lengths[L_index[1]]) // now L_index[0] <= L_index[1] <= L_index[2]
          {
            std::swap(L_index[0], L_index[1]);
          }


          // (step) mollify, by refence
          double a = edge_lengths[L_index[2]];
          double b = edge_lengths[L_index[1]];
          double c = edge_lengths[L_index[0]];
          c = (std::max)((std::max)(c, delta + a - b), delta + b - a);
          b = (std::max)(b, c);
          a = (std::max)(a, b);
          extra_length += (a - edge_lengths[L_index[2]]) + (b - edge_lengths[L_index[1]]) + (c - edge_lengths[L_index[0]]);
          edge_lengths[L_index[2]] = a;
          edge_lengths[L_index[1]] = b;
          edge_lengths[L_index[0]] = c;
          assert(c > 0 && b > 0 && a > 0);
        }
        if (scale_length)
        {
      ...
      void
      mollify_local_one_by_one_interpolated(const double delta, bool scale_length = false)
      {
        // compute smallest length epsilon we can add to
        // all edges to ensure that the strict triangle
        // inequality holds with a tolerance of delta
        ...
        for (face_descriptor f : faces(m_intrinsic_tm))
        {
          ...
          // reorder i, j, k so that edge_lengths[k] <= edge_lengths[j] <= edge_lengths[i]
          Index L_index[3] = {i, j, k};
          if (edge_lengths[L_index[0]] > edge_lengths[L_index[1]])
          {
            std::swap(L_index[0], L_index[1]);
          }
          if (edge_lengths[L_index[1]] > edge_lengths[L_index[2]])
          {
            std::swap(L_index[1], L_index[2]);
          }
          if (edge_lengths[L_index[0]] > edge_lengths[L_index[1]])
          ...
          // (interpolated) mollify, by refence
          double a = edge_lengths[L_index[2]];
          double b = edge_lengths[L_index[1]];
          double c = edge_lengths[L_index[0]];

          c = (std::max)((std::max)(c, delta + a - b), delta + b - a);
          if (a - edge_lengths[L_index[0]] > 1e-6 * delta)
          {
            b = (std::max)(c, b + (b - edge_lengths[L_index[0]]) / (a - edge_lengths[L_index[0]]) * delta);
          }
          else
          {
            b = (std::max)(b, c);
      ...
      build(VertexPointMap vpm)
      {
        ...
        copy_face_graph(m_input_tm, m_intrinsic_tm,
                        parameters::vertex_to_vertex_output_iterator(std::back_inserter(pairs)).vertex_point_map(vpm));
        ...
        if (broken_edges)
        {
          number_of_edges = num_halfedges(m_intrinsic_tm);
        }
        edge_lengths.resize(number_of_edges);
        // mark_edges.resize(number_of_edges, 1);
        edge_id_map = get(Edge_property_tag(), m_intrinsic_tm);
        // another map in case of broken edges
        halfedge_id_map = get(Halfedge_property_tag(), m_intrinsic_tm);
        Index edge_i = 0;
        VertexPointMap vpm_intrinsic_tm = get(boost::vertex_point, m_intrinsic_tm);

        // instead of min_length, we could use the average edge length
        for (edge_descriptor ed : edges(m_intrinsic_tm))
        {
          double edge_len = CGAL::sqrt(to_double(squared_distance(get(vpm_intrinsic_tm, source(ed, m_intrinsic_tm)),
          ...
          if (broken_edges)
          {
            ...
            put(halfedge_id_map, halfedge(ed, m_intrinsic_tm), edge_i * 2);
            put(halfedge_id_map, opposite(halfedge(ed, m_intrinsic_tm), m_intrinsic_tm), edge_i * 2 + 1);
            edge_lengths[edge_i*2] = edge_len;
            edge_lengths[edge_i*2+1] = edge_len;
            edge_i++;
          }
          else
          {
            edge_lengths[edge_i] = edge_len;
            total_length += edge_lengths[edge_i];
            put(edge_id_map, ed, edge_i);
            edge_i++;
          }
          // stack.push(ed);
        }
        double avg_length = total_length / number_of_edges;

        switch (m_mollification_type)
        {
        case NONE:
          break;
        case CONSTANT_EPSILON:
          mollify_constant(avg_length * delta, true);
          break;
        case LOCAL_CONSTANT_EPSILON:
          mollify_local_constant(avg_length * delta, true);
          break;
        case LOCAL_ONE_BY_ONE_STEP:
          mollify_local_one_by_one_step(avg_length * delta, true);
          break;
        case LOCAL_ONE_BY_ONE_INTERPOLATION:
          mollify_local_one_by_one_interpolated(avg_length * delta, true);
          break;
        default:
          // TODO: implement other mollification strategies
          break;
        }
        ...
        for (face_descriptor f : faces(m_intrinsic_tm))
        {
          ...
          if (broken_edges)
          {
            // will use halfedge_id_map to index into edge_lengths
            halfedge_descriptor hd1 = next(hd, m_intrinsic_tm);
            halfedge_descriptor hd2 = next(hd1, m_intrinsic_tm);
            halfedge_descriptor hd3 = next(hd2, m_intrinsic_tm);
            ...
            Point_2 p21(edge_lengths[get(halfedge_id_map, hd1)], 0);
            put(hcm, hd1, p21);

            // use basic trigonometry to compute third coordinate
            Index e1 = get(halfedge_id_map, hd1);
            Index e2 = get(halfedge_id_map, hd2);
            Index e3 = get(halfedge_id_map, hd3);
          ...
          {
            halfedge_descriptor hd1 = next(hd, m_intrinsic_tm);
            edge_descriptor ed1 = edge(hd1, m_intrinsic_tm);
            halfedge_descriptor hd2 = next(hd1, m_intrinsic_tm);
            edge_descriptor ed2 = edge(hd2, m_intrinsic_tm);
            halfedge_descriptor hd3 = next(hd2, m_intrinsic_tm);
            edge_descriptor ed3 = edge(hd3, m_intrinsic_tm);
      ...
      Halfedge_id_map halfedge_id_map;


      enum MOLLIFICATION_TYPE m_mollification_type;
      bool broken_edges;
      double delta;
      double total_length;
...
namespace CGAL
{
  namespace Heat_method_3
  {
    ...
    template <typename TM,
              typename T>
    typename boost::graph_traits<TM>::halfedges_size_type
    num_halfedges(const Intrinsic_Delaunay_triangulation_3<TM, T> &idt)
    {
      return num_halfedges(idt.triangle_mesh());
    }
